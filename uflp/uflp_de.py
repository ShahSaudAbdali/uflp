...
def uflp_objective(solution, customers, facilities, f_cost, c_cost):
...
def differential_evolution_uflp(customers, facilities, f_cost, c_cost, population_size=50, max_generations=100, F=0.8, CR=0.5):
    ...
    population = np.random.randint(2, size=(population_size, facilities, customers))
    ...
    for gen in range(max_generations):
        for i in range(population_size):
            ...
            # Evaluate fitness
            trial_fitness = uflp_objective(trial_solution, customers, facilities, f_cost, c_cost)
            current_fitness = uflp_objective(population[i], customers, facilities, f_cost, c_cost)

            # Selection
            if trial_fitness < current_fitness:
                population[i] = trial_solution

    best_solution = population[np.argmin([uflp_objective(sol, customers, facilities, f_cost, c_cost) for sol in population])]
...
if __name__ == "__main__":
    ...
    print("Facility Opening Cost Array: \n", f_cost)
    ...
    # print("Total cost of the solution:", uflp_objective(best_solution, customers, facilities, f_cost, c_cost))
